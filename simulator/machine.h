...
public:
    ...
    // Run simulation.
    void run(unsigned start_addr, unsigned finish_addr = 0);
    ...
    static void trace_display(unsigned level, unsigned addr)
    {
        if (debug_flag)
            print_display(level, addr);
    }
    ...
    static void print_display(unsigned level, unsigned addr);
};
